...
const localData =
{
  ...
  vr_order: {
    already_has_another_benefit: true,
    benefit_name: "Teste",
    number_of_beneficiaries: 1000,
    whats_va: true,
    average_value_per_employee_va: 1200,
    whats_vr: false,
    average_value_per_employee_vr: 0,
    whats_vale_auto: true,
    average_value_per_employee_vale_auto: 500,
  },
  whatsapp: {
    address: null,
    avatar: "",
        ...
        <div className="mt-4 text-neutral-700">
          <div className="bg-white rounded-md p-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {localData.order_type === "BENEFICIOS" &&
                vrOrder?.["already_has_another_benefit"] !== undefined && (
                  <>
                    <DisplayGenerator
                      title="Já possui outro benefício:"
                      value={formatYesNo(vrOrder?.["already_has_another_benefit"])}
                    />
                    <DisplayGenerator
                      title="Nome do benefício:"
                      value={(vrOrder?.["benefit_name"] as string) || "-"}
                    />
                    <DisplayGenerator
                      title="Número de beneficiários:"
                      value={String(vrOrder?.["number_of_beneficiaries"] ?? "-")}
                    />
                    <p></p>
                    <DisplayGenerator
                      title="Vale alimentação (VA):"
                      value={formatYesNo(vrOrder?.["whats_va"])}
                    />
                    <DisplayGenerator
                      title="Valor médio por colaborador (VA):"
                      value={formatBRL(vrOrder?.["average_value_per_employee_va"]) || "-"}
                    />
                    <DisplayGenerator
                      title="Vale refeição (VR):"
                      value={formatYesNo(vrOrder?.["whats_vr"])}
                    />
                    <DisplayGenerator
                      title="Valor médio por colaborador (VR):"
                      value={formatBRL(vrOrder?.["average_value_per_employee_vr"]) || "-"}
                    />
                    <DisplayGenerator
                      title="Vale combustível/auto:"
                      value={formatYesNo(vrOrder?.["whats_vale_auto"])}
                    />
                    <DisplayGenerator
                      title="Valor médio por colaborador (Vale Auto):"
                      value={formatBRL(vrOrder?.["average_value_per_employee_vale_auto"]) || "-"}
                    />
                  </>
                )}
            </div>
